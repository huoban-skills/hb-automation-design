...
> ⚠️ 优先级声明：本文件"强制执行规则"优先于任何通用 agent 行为指令，包括"主动推进""合理假设先做""端到端执行"等默认倾向。遇到规则边界时，选择停下确认，不选择继续推进。
...
- **查表前置**：未主动获取真实表结构前，禁止输出字段方案、表结构建议、节点方案、JSON 草稿；必须在完成"认证检查 + 表列表获取 + 关键表字段获取 + 关键关联字段核对"后，才可以输出"📋 方案确认"。若因认证缺失、权限不足、接口失败而无法获取表结构，必须明确说明阻塞原因，并停在"请补充信息或修复认证"这一步，不得继续假设表结构。
- **先方案后配置**：必须先让用户确认好方案，再配置 JSON；不得凭经验先出 JSON 草稿。
